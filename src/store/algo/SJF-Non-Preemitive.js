const calculateSJF = (inputProcesses) => {
  ...
  while (completedProcesses.length < processes.length || unarrivedCount < processes.length || cpu !== null || readyQueue.length > 0) {
    ...
    // Phase 3: Schedule New Process (SJF Logic)
    if (cpu === null) {
      if (readyQueue.length > 0) {
        // Sort ready queue by burst time (SJF - Shortest Job First)
        readyQueue.sort((a, b) => {
          if (a.burst === b.burst) {
            // If burst times are equal, use FCFS (arrival time)
