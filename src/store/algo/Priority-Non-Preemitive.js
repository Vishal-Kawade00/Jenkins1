const calculatePriority = (inputProcesses) => {
  ...
  while (completedProcesses.length < processes.length || unarrivedCount < processes.length || cpu !== null || readyQueue.length > 0) {
    ...
    // Phase 3: Schedule New Process (Priority Logic)
    if (cpu === null) {
      if (readyQueue.length > 0) {
        // Sort ready queue by priority (assuming lower number = higher priority)
        readyQueue.sort((a, b) => {
          // Primary sort: by priority (lower number = higher priority)
          if (a.priority !== b.priority) {
            return a.priority - b.priority;
          }
          // Secondary sort: if same priority, use FCFS (arrival time)
    ...
    // Phase 4: Advance time and decrement burst
    if (cpu) {
